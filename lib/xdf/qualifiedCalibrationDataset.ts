...
import {
  ...
  createQualifiedCalibrationExtractionBatchContext,
...
function materializeDefinition(input:{request:QualifiedCalibrationDatasetRequest;authority:RomLayoutMembershipAuthority;membership:QualifiedBinaryToRomLayoutMembership;definition:XdfDefinitionRevision;batchContext:ReturnType<typeof createQualifiedCalibrationExtractionBatchContext>}):CalibrationDatasetDefinition{
  const {request,authority,membership,definition,batchContext}=input;
  const preflight=classifyUnmaterializableDatasetDefinition(definition);if(preflight)return preflight;
  const extracted=extractQualifiedCalibrationValue(constructQualifiedCalibrationExtractionRequest({engineeringBinary:request.engineeringBinary,binaryIdentity:request.binaryIdentity,romLayoutId:authority.layout.layoutId,registrySnapshot:request.applicabilityRegistry,expectedRelationshipId:authority.relationship.relationshipId,expectedRelationshipRevision:authority.relationship.relationshipRevision,definitionSet:authority.definitionSet,definition,binaryLayoutMembership:membership,batchContext}));
