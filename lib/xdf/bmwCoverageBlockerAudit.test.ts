...
test("B58 Gen1 repository census exhausts binaries and proves candidate-specific exact markers", { timeout: 900_000 }, (context) => { const audit = auditFamily("B58gen1"); assert.equal(audit.sources.length, 7); assert.equal(audit.sources.reduce((sum, item) => sum + item.binaries.length, 0), 11); assert.equal(audit.sources.filter((item) => item.binaries.length === 0).map((item) => item.romSoftwareIdentity).join(), "000030765A5005"); assert.equal(audit.sources.reduce((sum, item) => sum + item.conflicts.reduce((n, conflict) => n + conflict.definitionRevision.split(" + ").length, 0), 0), 4); assert.ok(audit.sources.flatMap((item) => item.binaries).every((binary) => binary.extraction.attempted === binary.extraction.successful + binary.extraction.blocked)); assert.ok(audit.sources.every((source) => source.binaries.every((binary) => binary.markers.length > 0 && binary.markers.every((marker) => marker.identifier === source.romSoftwareIdentity && marker.classification === "candidate_specific")))); assert.equal(audit.authorityGranted, false); context.diagnostic(`B58_GEN1_BLOCKER_AUDIT ${JSON.stringify(audit)}`); });
