...
export function extractQualifiedCalibrationValue(request:QualifiedCalibrationExtractionRequest):QualifiedCalibrationExtractionResult{
  const binaryBytes=request.engineeringBinary.bytes;const exactDigest=createHash("sha256").update(binaryBytes).digest("hex");const lookup=lookupActiveRomLayoutApplicability(request.registrySnapshot,request.romLayoutId);
  if(exactDigest!==request.binaryIdentity.digest||request.binaryIdentity.byteLength!==binaryBytes.byteLength||request.binaryIdentity.identityId!==`engineering-binary:${exactDigest}`)return rejected(request,lookup,null,"binary_identity_mismatch","Engineering Binary bytes do not match the supplied exact Binary Identity.");
