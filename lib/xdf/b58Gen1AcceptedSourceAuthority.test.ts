...
import { materializeB58Gen1AcceptedApplicabilityDecision } from "./b58Gen1AcceptedApplicabilityDecision.ts";
...
test("accepted 00007972000705 Source Authority is exact and advances only to an unpublished applicability candidate", { timeout: 180_000 }, (context) => {
  ...
  const accepted = materializeB58Gen1AcceptedApplicabilityDecision({ candidate, proposal, assessment });
  assert.equal(accepted.decision.outcome, "accept_exact_applicability");
  assert.equal(accepted.decision.publicationState, "not_published");
  assert.equal(accepted.workshopAdmission, "not_authorized");
  assert.equal(accepted.stockAuthenticity, "not_assessed");
  assert.deepEqual(accepted.publicationSideEffects, []);
  context.diagnostic(`B58_GEN1_00007972000705_ACCEPTED_SOURCE_AUTHORITY ${JSON.stringify({ authority: B58_GEN1_00007972000705_SOURCE_AUTHORITY, evidencePackageRevision: evidencePackage.packageRevision, assessment, candidate, accepted })}`);
});
