...
export type AdmissionLifecycleChangeOperation = "supersede" | "deprecate" | "restore" | "reject" | "record_dispute" | "resolve_conflict";
export type AdmissionLifecycleChange = Readonly<{
  changeId: string;
  sequence: number;
  operation: AdmissionLifecycleChangeOperation;
  targetStableKnowledgeId: string;
  targetKnowledgeVersion: string;
  priorStatus: KnowledgeLifecycleStatus;
  proposedStatus: KnowledgeLifecycleStatus;
  reason: GovernedVocabularyReference;
  relatedStableKnowledgeIds: readonly string[];
}>;
export type AdmissionLifecycleChangeManifest = Readonly<{ changes: readonly AdmissionLifecycleChange[] }>;
...
const lifecycleChangeOperations = new Set<AdmissionLifecycleChangeOperation>(["supersede", "deprecate", "restore", "reject", "record_dispute", "resolve_conflict"]);
const knowledgeLifecycleStatuses = new Set<KnowledgeLifecycleStatus>(["active", "superseded", "deprecated", "rejected"]);
...
function validateLifecycleChangeManifest(value: AdmissionLifecycleChangeManifest, stableKnowledgeId: string): void { requireUnique(value.changes.map((item) => item.changeId), "Lifecycle change identity"); const transitions = new Set<string>(); const sequences = new Set<number>(); for (const item of value.changes) { requireNonBlank(item.changeId, "Lifecycle change identity"); if (!Number.isSafeInteger(item.sequence) || item.sequence < 1) throw new Error("Lifecycle change sequence must be a positive safe integer."); if (sequences.has(item.sequence)) throw new Error(`Lifecycle change sequence contains duplicate ${item.sequence}.`); sequences.add(item.sequence); requireKnown(item.operation, lifecycleChangeOperations, "Lifecycle change operation"); requireNonBlank(item.targetStableKnowledgeId, "Lifecycle target Knowledge identity"); requireNonBlank(item.targetKnowledgeVersion, "Lifecycle target Knowledge version"); if (item.targetStableKnowledgeId !== stableKnowledgeId) throw new Error("Lifecycle change targets unrelated Knowledge identity."); requireKnown(item.priorStatus, knowledgeLifecycleStatuses, "Prior Knowledge lifecycle status"); requireKnown(item.proposedStatus, knowledgeLifecycleStatuses, "Proposed Knowledge lifecycle status"); validateVocabulary(item.reason, "Lifecycle change reason"); requireUnique(item.relatedStableKnowledgeIds, "Related lifecycle Knowledge identity"); const transition = `${item.targetStableKnowledgeId}:${item.targetKnowledgeVersion}:${item.priorStatus}:${item.proposedStatus}`; if (transitions.has(transition)) throw new Error("Lifecycle manifest contains duplicate contradictory transition."); transitions.add(transition); const valid = item.operation === "supersede" ? item.priorStatus === "active" && item.proposedStatus === "superseded" : item.operation === "deprecate" ? item.priorStatus === "active" && item.proposedStatus === "deprecated" : item.operation === "reject" ? item.priorStatus === "active" && item.proposedStatus === "rejected" : item.operation === "restore" ? item.priorStatus !== "active" && item.proposedStatus === "active" : item.priorStatus === item.proposedStatus; if (!valid) throw new Error(`Lifecycle transition is invalid for operation ${item.operation}.`); } }
...
export function defineProposedCalibrationKnowledgeRevision(input: ProposedCalibrationKnowledgeRevision): ProposedCalibrationKnowledgeRevision { const validated = construct(input, (value) => { validateIdentity(value.revisionIdentity, "Proposed Knowledge revision"); defineCalibrationKnowledgeObject(value.knowledge); requireNonBlank(value.proposedStableKnowledgeId, "Proposed stable Knowledge identity"); requireNonBlank(value.proposedKnowledgeVersion, "Proposed Knowledge version"); if ((value.expectedPredecessorStableKnowledgeId === null) !== (value.expectedPredecessorKnowledgeVersion === null)) throw new Error("Predecessor stable identity and Knowledge version must be supplied together."); if (value.expectedPredecessorStableKnowledgeId !== null) requireNonBlank(value.expectedPredecessorStableKnowledgeId, "Expected predecessor stable Knowledge identity"); if (value.expectedPredecessorKnowledgeVersion !== null) requireNonBlank(value.expectedPredecessorKnowledgeVersion, "Expected predecessor Knowledge version"); if (value.knowledge.identity.value?.stableId !== value.proposedStableKnowledgeId) throw new Error("Proposed stable Knowledge identity must match the canonical object identity."); if (value.knowledge.version !== value.proposedKnowledgeVersion) throw new Error("Proposed Knowledge version must match the canonical object version."); validateManifest(value.changeManifest); validateLifecycleChangeManifest(value.lifecycleChangeManifest, value.proposedStableKnowledgeId); requireNonBlank(value.publicationRationale, "Publication rationale"); }); return deepCloneFreeze({ ...validated, lifecycleChangeManifest: { changes: [...validated.lifecycleChangeManifest.changes].sort((left, right) => left.sequence - right.sequence || left.changeId.localeCompare(right.changeId)) } }); }
...
export function defineCalibrationKnowledgeAdmissionProposal(input: CalibrationKnowledgeAdmissionProposal): CalibrationKnowledgeAdmissionProposal { return construct(input, (value) => { validateIdentity(value.proposalIdentity, "Admission proposal"); requireKnown(value.proposalKind, proposalKinds, "Admission proposal kind"); defineProposedCalibrationKnowledgeRevision(value.proposedRevision); validateIdentity(value.evidencePackageIdentity, "Admission evidence package reference"); requireUnique(value.authorityClaimIds, "Admission authority claim identity"); validateAuthority(value.proposer, "Proposal"); validateVocabulary(value.proposerSourceRole, "Proposer source role"); validateVocabulary(value.intendedPublicationClass, "Intended publication class"); requireTimestamp(value.submittedAt, "Proposal submission time"); requireNonBlank(value.contractVersion, "Admission proposal contract version"); requireNonBlank(value.rationale, "Admission proposal rationale"); value.unresolvedQuestions.forEach((item) => requireNonBlank(item, "Admission unresolved question")); validateLifecycle(value.lifecycle, "Admission proposal"); if (value.lifecycle.status === "draft") throw new Error("Submitted Admission Proposal cannot retain draft lifecycle."); requireLifecycleStatus(value.lifecycle, ["submitted", "superseded"], "Admission proposal"); const predecessorPresent = value.proposedRevision.expectedPredecessorStableKnowledgeId !== null && value.proposedRevision.expectedPredecessorKnowledgeVersion !== null; if (value.proposalKind === "register_new" && predecessorPresent) throw new Error("New registration cannot declare a predecessor Knowledge binding."); if (value.proposalKind !== "register_new" && !predecessorPresent) throw new Error("Amendment proposal requires a predecessor Knowledge identity and version."); const expectedLifecycleOperation: Partial<Record<AdmissionProposalKind, AdmissionLifecycleChangeOperation>> = { supersede: "supersede", deprecate: "deprecate", restore: "restore", reject: "reject", record_dispute: "record_dispute", resolve_conflict: "resolve_conflict" }; const expected = expectedLifecycleOperation[value.proposalKind]; const changes = value.proposedRevision.lifecycleChangeManifest.changes; if (expected === undefined && changes.length > 0) throw new Error(`Proposal kind ${value.proposalKind} cannot declare lifecycle changes.`); if (expected !== undefined && (changes.length === 0 || changes.some((item) => item.operation !== expected))) throw new Error(`Proposal kind ${value.proposalKind} requires lifecycle operation ${expected}.`); }); }
