...
export function auditRepositoryWorkingCalibrationCapabilities(): WorkingCalibrationCapabilityAudit {
  ...
  return Object.freeze({ relationships: entries.length, tables: definitions.length, technicallyReversible, inverseBlocked, quarantined, unavailable: inverseBlocked + quarantined, editQualified: 0, viewOnly: definitions.length });
}
