...
export async function loadSubscriberCalibration(input: Readonly<{ bytes: Uint8Array; fileName: string; mimeType: string | null; selectedDefinition?: string | null; observedAt?: string }>): Promise<SubscriberCalibrationResult> {
  ...
  const binaryIdentity = identifyEngineeringBinary({ engineeringBinary: binary, romFamily: entry.identity.family, softwareIdentity: identity, calibrationIdentity: identity, internalRomIdentifiers: observations.map((item) => item.normalizedForm), identityProvenance: [observation.observationId] });
