...
export type WorkshopKnowledgeRecord = Readonly<{
  ...
  calibrationRole?: QualifiedSemanticField<string> | null;
  ...
  outputMeaning?: QualifiedSemanticField<string> | null;
...
export function bindDefinitionKnowledge(definition: Readonly<{ key?: string; definitionIdentity: string | null; definitionRevision: string }>, records: readonly WorkshopKnowledgeRecord[]): WorkshopSemanticBinding {
  ...
  const fields = [record.engineeringSystem, record.calibrationRole ?? null, record.outputMeaning ?? null, ...(record.relatedEngineeringSystems ?? []), ...record.controls, ...record.whyItMatters, ...record.howToRead, ...record.directionalEffects, ...record.operatingContexts, ...record.engineeringConsiderations, ...record.axisMeanings, ...record.relatedCalibrations, ...record.telemetryRelationships].filter((field): field is NonNullable<typeof field> => field !== null);
  return Object.freeze({ outcome, candidates: Object.freeze([record.knowledgeId]), knowledgeId: record.knowledgeId, knowledgeRevision: record.knowledgeRevision, engineeringSystem: record.engineeringSystem, relatedEngineeringSystems: Object.freeze([...(record.relatedEngineeringSystems ?? [])]), calibrationRole: record.calibrationRole ?? null, sourceSymbol: record.sourceSymbol ?? null, controls: record.controls, whyItMatters: record.whyItMatters, howToRead: record.howToRead, directionalEffects: record.directionalEffects, operatingContexts: record.operatingContexts, engineeringConsiderations: record.engineeringConsiderations, axisMeanings: record.axisMeanings, outputMeaning: record.outputMeaning ?? null, relatedCalibrations: record.relatedCalibrations, telemetryRelationships: record.telemetryRelationships, aliases: Object.freeze(record.aliases.map((alias) => alias.value)), provenance: Object.freeze([...new Set(fields.flatMap((field) => field.provenance))]), limitations: Object.freeze([...new Set([...record.limitations, ...fields.flatMap((field) => field.limitations)])]), unavailableReason: outcome === "partial" ? "Definition-specific engineering interpretation remains incomplete." : null });
}
