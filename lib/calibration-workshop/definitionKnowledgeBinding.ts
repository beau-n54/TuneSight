...
const empty = (outcome: WorkshopSemanticOutcome, candidates: readonly string[], reason: string): WorkshopSemanticBinding => Object.freeze({ outcome, candidates: Object.freeze([...candidates]), knowledgeId: null, knowledgeRevision: null, engineeringSystem: null, controls: Object.freeze([]), whyItMatters: Object.freeze([]), howToRead: Object.freeze([]), axisMeanings: Object.freeze([]), relatedCalibrations: Object.freeze([]), telemetryRelationships: Object.freeze([]), aliases: Object.freeze([]), provenance: Object.freeze([]), limitations: Object.freeze([]), unavailableReason: reason });

export function bindDefinitionKnowledge(definition: Readonly<{ definitionIdentity: string | null; definitionRevision: string }>, records: readonly WorkshopKnowledgeRecord[]): WorkshopSemanticBinding {
  const exact = records.filter((record) => record.lifecycle === "active" && ((definition.definitionIdentity !== null && record.exactDefinitionIdentities.includes(definition.definitionIdentity)) || record.exactDefinitionRevisions.includes(definition.definitionRevision)));
  ...
  const contextual = records.filter((record) => record.lifecycle === "active" && definition.definitionIdentity !== null && record.applicability.includes(definition.definitionIdentity));
  ...
  const fields = [record.engineeringSystem, ...record.controls, ...record.whyItMatters, ...record.howToRead, ...record.axisMeanings, ...record.relatedCalibrations, ...record.telemetryRelationships].filter((field): field is NonNullable<typeof field> => field !== null);
  return Object.freeze({ outcome, candidates: Object.freeze([record.knowledgeId]), knowledgeId: record.knowledgeId, knowledgeRevision: record.knowledgeRevision, engineeringSystem: record.engineeringSystem, controls: record.controls, whyItMatters: record.whyItMatters, howToRead: record.howToRead, axisMeanings: record.axisMeanings, relatedCalibrations: record.relatedCalibrations, telemetryRelationships: record.telemetryRelationships, aliases: Object.freeze(record.aliases.map((alias) => alias.value)), provenance: Object.freeze([...new Set(fields.flatMap((field) => field.provenance))]), limitations: Object.freeze([...new Set([...record.limitations, ...fields.flatMap((field) => field.limitations)])]), unavailableReason: outcome === "partial" ? "Definition-specific engineering interpretation remains incomplete." : null });
}
