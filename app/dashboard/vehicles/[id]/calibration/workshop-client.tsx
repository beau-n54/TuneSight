        ...
        {!detail.summary.available ? (
          ...
          <div className="mt-5 overflow-x-auto pb-2">
            <table className="min-w-max border-separate border-spacing-1 text-xs">
              <thead><tr><th className="sticky left-0 z-10 bg-zinc-950 p-2 text-left text-zinc-500">{rowAxis?.id || "Row"} \ {columnAxis?.id || "Column"}</th>{Array.from({ length: detail.columns }, (_, column) => <th key={column} className="min-w-28 p-2 font-mono font-normal text-zinc-400">{String(columnAxis?.values[column] ?? column)}{columnAxis?.units ? ` ${columnAxis.units}` : ""}</th>)}</tr></thead>
              <tbody>{Array.from({ length: detail.rows }, (_, row) => <tr key={row}><th className="sticky left-0 z-10 bg-zinc-950 p-2 text-left font-mono font-normal text-zinc-400">{String(rowAxis?.values[row] ?? row)}{rowAxis?.units ? ` ${rowAxis.units}` : ""}</th>{Array.from({ length: detail.columns }, (_, column) => { const index = row * detail.columns + column; const item = detail.cells[index]; if (!item) return <td key={column} className="border border-zinc-800 p-3 text-zinc-600">—</td>; return <td key={column}><button type="button" onClick={() => setSelectedCell(index)} className={`w-full min-w-28 rounded-lg border p-2 text-left font-mono transition ${index === selectedCell ? "border-blue-300 ring-1 ring-blue-300/40" : item.changed ? "border-blue-400/35 bg-blue-400/10 hover:border-blue-300" : "border-zinc-800 bg-zinc-900 hover:border-zinc-600"}`}><span className="block text-[10px] text-zinc-500">REF {formatValue(item.referenceValue)}</span><span className="mt-1 block text-zinc-100">CUR {formatValue(item.currentValue)}</span></button></td>; })}</tr>)}</tbody>
            </table>
          </div>
        )}
      </div>
