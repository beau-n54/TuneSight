...
import { moveSelectedCell, type CalibrationVisualState, type CalibrationVisualizationModel } from "@/lib/calibration-workshop/visualizationModel";
import type { WorkshopDefinitionDetail } from "@/lib/calibration-workshop/viewModel";

type Projected = { cellIndex: number; x: number; y: number; changed: boolean; row: number; column: number };
...
export default function CalibrationSurface3D({ model, detail, state, selectedCell, onSelect }: {
  ...
  useEffect(() => {
    ...
    const projected = model.surface.map((point) => {
      const gx = point.column - (detail.columns - 1) / 2, gy = point.row - (detail.rows - 1) / 2;
      const rx = gx * cos - gy * sin, depth = gx * sin + gy * cos;
      const value = state === "reference" ? point.referenceValue : point.currentValue;
      const elevation = (value - minimum) / range * Math.min(rect.height * 0.38, 190);
      return { cellIndex: point.cellIndex, x: centerX + rx * scale, y: centerY + depth * scale * Math.sin(tilt) - elevation * Math.cos(tilt), changed: point.changed, row: point.row, column: point.column };
    });
    ...
    context.fillText(`Height: values${model.valueUnits ? ` (${model.valueUnits})` : ""}`, rect.width - 220, 24);
  }, [model, detail, state, selectedCell, values, rotation, tilt, zoom]);
