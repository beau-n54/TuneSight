...
create table if not exists public.acquired_xdf_source_artifacts (
  id uuid primary key default gen_random_uuid(), source_record_id text not null unique, source_digest text not null unique check (source_digest ~ '^[0-9a-f]{64}$'), byte_length integer not null check (byte_length between 1 and 4194304),
